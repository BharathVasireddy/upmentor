...
export async function PUT(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ user: { id: user.id, interests }, challenges, goals }, { status: 200 });
  } catch (error) {
    return NextResponse.json({ error: 'Failed to update interests and challenges' }, { status: 500 });
  }
}
